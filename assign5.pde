...
final float CLOCK_BONUS_SECONDS = 60f * 15 ;
...
void initClocks(){
  ...
  for(int i = 0; i < cabbageX.length; i++){
    ...
    if( clockX[i] == cabbageX[i] && clockY[i] == cabbageY[i]){ clockX[i] = SOIL_SIZE * floor(random(SOIL_COL_COUNT));
    clockY[i] = SOIL_SIZE * ( i * 4 + floor(random(4))); }
  }
}
...
void draw() {
	...
	switch (gameState) {
		...
		for(int i = 0; i < cabbageX.length; i++){
			...
			if(isHit( cabbageX[i], cabbageY[i], cabbageX[i]+SOIL_SIZE, cabbageY[i]+SOIL_SIZE, playerX, playerY, playerX+SOIL_SIZE, playerY+SOIL_SIZE)) { // r1 bottom edge past r2 top
    ...
    for(int i = 0; i < clockX.length; i++){
      ...
      if(isHit( clockX[i], clockY[i], clockX[i]+SOIL_SIZE, clockY[i]+SOIL_SIZE, playerX, playerY, playerX+SOIL_SIZE, playerY+SOIL_SIZE)) { // r1 bottom edge past r2 top
		...
		for(int i = 0; i < soldierX.length; i++){
			...
			// Requirement #3: Use boolean isHit(...) to detect collision
			if( isHit( soldierX[i], soldierY[i], soldierX[i]+SOIL_SIZE, soldierY[i]+SOIL_SIZE, playerX, playerY, playerX+SOIL_SIZE, playerY+SOIL_SIZE)) { // r1 bottom edge past r2 top
...
void addTime(float frames){					// Requirement #2
  gameTimer += frames;
}
...
color getTimeTextColor(int frames){				// Requirement #5
  ...
  else if(frames < 600 ){ return #ff0000; }
